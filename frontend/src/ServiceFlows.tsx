...
function ShipBarrelFlow() {
  ...
  const fillPercent = Math.min(100, Math.round((volume / selectedBarrel.volumeIn3) * 100));
...
function ExpressGiftsFlow() {
  ...
  const paymentReady = payment.name.trim().length > 1 && payment.number.replace(/\D/g, '').length >= 12 && payment.expiry.trim().length >= 4 && payment.cvc.replace(/\D/g, '').length >= 3;
  ...
  function updatePayment(key: keyof typeof payment, value: string) { setPayment({ ...payment, [key]: value }); }
  function add(item: CatalogItem) { const variant = item.variants.find(v => v.id === selected[item.id]); if (item.variants.length && !variant) return; persist([...cart, { productId: item.id, name: item.name, qty: 1, price: variant?.price ?? item.price, sizeLabel: variant?.label, delivery: item.deliverySpeed, image: item.image }]); setDone(''); }
  ...
  async function confirm(e: FormEvent) {
    ...
    if (!address) return;
    if (!paymentReady) { setError('Enter the dummy payment details before creating the gift order.'); return; }
    setBusy(true);
    setError('');
    try {
      const paymentMethod = { provider: 'dummy', cardholderName: payment.name.trim(), last4: payment.number.replace(/\D/g, '').slice(-4), status: 'authorized_for_demo' };
      const order = await api<{ model: { _id: string }; total: number; message: string }>('/site/gift-checkout', { method: 'POST', auth: true, body: JSON.stringify({ items: cart, address, instructions, occasion, surpriseNote, submittedTotal: subtotal, paymentMethod }) });
      persist([]);
