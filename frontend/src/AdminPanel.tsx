...
export default function AdminPanel(){
  ...
  if(section.resource==='item-catalog')return <section className="admin-shell"><aside className="admin-sidebar"><Link className="brand light" to="/">MIRIAX ONE</Link><small>ADMIN CONSOLE</small>{adminSections.map(item=><button key={item.resource} disabled={busy} className={section.resource===item.resource?'active':''} onClick={()=>navigate(item.resource)}>{item.label}</button>)}<Link className="back-site" to="/account">Account settings</Link><Link className="back-site" to="/">Back to website</Link></aside><ItemCatalogAdmin/></section>;
  if(section.resource==='dashboard')return <section className="admin-shell"><aside className="admin-sidebar"><Link className="brand light" to="/">MIRIAX ONE</Link><small>ADMIN CONSOLE</small>{adminSections.map(item=><button key={item.resource} disabled={busy} className={section.resource===item.resource?'active':''} onClick={()=>navigate(item.resource)}>{item.label}</button>)}<Link className="back-site" to="/account">Account settings</Link><Link className="back-site" to="/">Back to website</Link></aside><div className="admin-main"><div className="admin-top"><div><span className="eyebrow">Operations overview</span><h1>Dashboard</h1><p>Live KPIs for orders, revenue, pending shipments, and delivered count will appear here as analytics mature.</p></div><SignOutButton/></div><div className="metrics"><div><b>Total orders</b><span>Connected to Orders</span></div><div><b>Revenue</b><span>Current period</span></div><div><b>Pending</b><span>Needs action</span></div><div><b>Delivered</b><span>Completed shipments</span></div></div></div></section>;
