...
function AdminSidebar({section,busy,navigate}:{section:{resource:string;label:string};busy:boolean;navigate:(resource:string)=>void}) {
  return <aside className="admin-sidebar"><Link className="brand light" to="/">MIRIAX ONE</Link><small>ADMIN CONSOLE</small><div className="admin-nav-groups">{adminGroups.map(group=><details key={group.label} open={group.items.includes(section.resource)}><summary>{group.label}</summary>{adminSections.filter(item=>group.items.includes(item.resource)).map(item=><button key={item.resource} disabled={busy} className={section.resource===item.resource?'active':''} onClick={()=>navigate(item.resource)}>{item.label}</button>)}</details>)}</div><Link className="back-site" to="/account">Account settings</Link><Link className="back-site" to="/">Back to website</Link></aside>;
}

export default function AdminPanel(){
  const [params,setParams]=useSearchParams();
  const section=adminSections.find(item=>item.resource===params.get('section'))||adminSections.find(item=>item.resource==='dashboard')!;
  ...
  useEffect(()=>{let live=true;if(specialSection){setLoading(false);setError('');return()=>{live=false}}setLoading(true);setError('');const controller=new AbortController();
  ...
  },[filterKey,revision,specialSection]);
  ...
  useEffect(()=>{let live=true;if(specialSection)return()=>{live=false};Promise.all([lookup('country'),lookup('shipping-type')]).then(([c,m])=>{if(live){setCountries(c);setMethods(m);setLookupError('')}}).catch(e=>{if(live)setLookupError(e.message)});return()=>{live=false}},[revision]);
  ...
  useEffect(()=>{requestId.current++;setEditor(undefined);setDetail(null);setBusy(false);setSelected([])},[section.resource]);
  useEffect(()=>{setSelected([])},[filterKey,archived]);
  function navigate(resource:string){requestId.current++;setParams({section:resource});setPage(1);setQuery('');setSearch('');setStatus('');setCountryFilter('');setArchived(false);setSort('-createdAt');setEditor(undefined);setDetail(null);setSelected([]);setNotice('');setError('')}
  ...
  async function toggleField(record:AdminItem,key:string){setBusy(true);setError('');try{await api(`/admin/${section.resource}/${record._id}`,{method:'PUT',auth:true,body:JSON.stringify({[key]:record[key]===1?0:1})});setRevision(v=>v+1)}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
  async function toggle(record:AdminItem){await toggleField(record,'isActive')}
  function toggleSelected(id:string){setSelected(list=>list.includes(id)?list.filter(item=>item!==id):[...list,id])}
  function toggleAllVisible(){setSelected(list=>allVisibleSelected?list.filter(id=>!result.items.some(row=>String(row._id)===id)):[...new Set([...list,...result.items.map(row=>String(row._id))])])}
  async function bulkPut(payload:Record<string,unknown>,label:string){if(!selectedRows.length)return;setBusy(true);setError('');try{await Promise.all(selectedRows.map(row=>api(`/admin/${section.resource}/${row._id}`,{method:'PUT',auth:true,body:JSON.stringify(payload)})));setNotice(`${selectedRows.length} ${section.label.toLowerCase()} updated: ${label}.`);setSelected([]);setRevision(v=>v+1)}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
  async function bulkArchive(){if(!selectedRows.length)return;if(!archived&&!window.confirm(`Archive ${selectedRows.length} selected records? You can restore them from the archive.`))return;setBusy(true);setError('');try{await Promise.all(selectedRows.map(row=>api(`/admin/${section.resource}/${row._id}${archived?'/restore':''}`,{method:archived?'POST':'DELETE',auth:true})));setNotice(archived?'Selected records restored.':'Selected records archived.');setSelected([]);setRevision(v=>v+1)}catch(e){setError((e as Error).message)}finally{setBusy(false)}}
  ...
  return <section className="admin-shell"><AdminSidebar section={section} busy={busy} navigate={navigate}/>
    <div className="admin-main"><div className="admin-top"><div><span className="eyebrow">Operations</span><h1>{section.label}</h1></div><SignOutButton/></div>
      ...
      <div className="admin-panel"><div className="admin-panel-head"><h2>{section.label} records <small>({result.totalRecords})</small></h2>{!archived&&<button className="button" disabled={busy} onClick={()=>void open(null)}>New record</button>}</div>
        ...
        {selectedRows.length>0&&<div className="bulk-bar"><b>{selectedRows.length} selected</b>{activeCapable&&<><button className="table-action" disabled={busy} onClick={()=>void bulkPut({isActive:1},'activated')}>Activate</button><button className="table-action" disabled={busy} onClick={()=>void bulkPut({isActive:0},'deactivated')}>Deactivate</button></>}{section.resource==='country'&&<><button className="table-action" disabled={busy} onClick={()=>void bulkPut({canSendFrom:1},'origin enabled')}>Enable origin</button><button className="table-action" disabled={busy} onClick={()=>void bulkPut({canSendFrom:0},'origin disabled')}>Disable origin</button><button className="table-action" disabled={busy} onClick={()=>void bulkPut({canDeliverTo:1},'destination enabled')}>Enable destination</button><button className="table-action" disabled={busy} onClick={()=>void bulkPut({canDeliverTo:0},'destination disabled')}>Disable destination</button></>}<button className="table-action danger" disabled={busy} onClick={()=>void bulkArchive()}>{archived?'Restore selected':'Archive selected'}</button><button className="table-action" disabled={busy} onClick={()=>setSelected([])}>Clear</button></div>}
        {loading?<p role="status" className="admin-loading">Loading records...</p>:<div className="admin-table-wrap"><table><thead><tr><th className="select-col"><input type="checkbox" aria-label="Select all visible rows" checked={allVisibleSelected} onChange={toggleAllVisible}/></th>{section.columns.map(key=><th key={key}>{columnLabels[key]}</th>)}<th>Actions</th></tr></thead><tbody>{result.items.map(row=><tr key={row._id} className={selected.includes(String(row._id))?'selected-row':''}><td className="select-col"><input type="checkbox" aria-label={`Select ${String(row.name||row._id)}`} checked={selected.includes(String(row._id))} onChange={()=>toggleSelected(String(row._id))}/></td>{section.columns.map(key=><td key={key}>{display(row,key)}</td>)}<td><div className="actions"><button className="table-action" disabled={busy} onClick={()=>void open(row,false)}>View</button>{!archived&&<button className="table-action" disabled={busy} onClick={()=>void open(row)}>Edit</button>}{section.active&&!archived&&<button className="table-action" disabled={busy} onClick={()=>void toggle(row)}>{row.isActive===1?'Deactivate':'Activate'}</button>}<button className="table-action" disabled={busy} onClick={()=>void archive(row)}>{archived?'Restore':'Archive'}</button></div></td></tr>)}{!result.items.length&&<tr><td colSpan={section.columns.length+2}>No matching records.</td></tr>}</tbody></table></div>}
