...
async def configuration(db):
    ...
    destination_country_codes = {c['iso'] for c in countries if c.get('canDeliverTo')}
    cities = [dict(id=str(c.get('_id', '')), name=c['name'], countryCode=c['countryCode'], stateCode=c.get('stateCode', ''), kind='city')
              ...
              if c.get('countryCode') in destination_country_codes]
    ...
    cities += [dict(id=str(s.get('_id', '')), name=s['name'], countryCode=s['countryCode'], stateCode=s.get('isoCode', ''), kind='region')
               ...
               if s.get('countryCode') in destination_country_codes and s.get('countryCode') not in city_countries]
