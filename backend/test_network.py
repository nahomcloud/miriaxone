...
class ResolverTests(unittest.TestCase):
    def setUp(self):
        self.config = {'services':[dict(key='container',name='Container',status='active',visibility='visible')],
            'countries':[dict(iso='US',canSendFrom=1,canDeliverTo=1),dict(iso='ET',canSendFrom=1,canDeliverTo=1)],
            'managedNetwork':True,'routes':[dict(origin='US',destination='ET',service='container',status='active',visibility='visible',archived=False)]}

    def test_valid_route_and_legacy_compatibility(self):
        self.assertEqual(resolve(self.config,'US','ET','container'),'')
    ...
    def test_all_deny_rules_override_active_routes(self):
        for collection,field,value in [('services','status','suspended'),('services','visibility','hidden'),('countries','canSendFrom',0),('routes','archived',True),('routes','status','coming-soon'),('routes','visibility','hidden')]:
            ...
            self.assertTrue(resolve(config,'US','ET','container'),(collection,field))
        self.assertTrue(resolve(self.config,'ET','US','container'))
        self.assertTrue(resolve(self.config,'US','ET','gift'))
        self.config['countries'][1]['canDeliverTo']=0
        self.assertTrue(resolve(self.config,'US','ET','container'))
...
class NetworkApiTests(unittest.TestCase):
    ...
    def test_checkout_cannot_bypass_disabled_service(self):
        ...
        self.db.platformServices.records.append(dict(key='container',name='Container',status='suspended',visibility='visible'))
    ...
    def test_network_persistence_validation_and_public_availability(self):
        service=dict(key='container',name='Fill a Container',status='suspended',visibility='visible')
        ...
        self.assertEqual(self.client.put('/admin/service/'+created.json()['_id'],json={'key':'ship-items'},headers=self.headers).status_code,422)
        availability=self.client.get('/site/availability?origin=ET&destination=ET').json()
        self.assertFalse(next(s for s in availability if s['key']=='container')['available'])
        route=dict(name='Test route',origin='ET',destination='ET',service='container',status='active',visibility='visible')
